...
void SyncResources(SOCKET Sock){
	...
	for(auto FN = FNames.begin(),FS = FSizes.begin(); FN != FNames.end() && !Terminate; ++FN,++FS) {
		...
		if (fs::exists(a)) {
			...
			if (fs::file_size(a) == std::stoull(*FS)){
				...
				try {
					...
					fs::copy_file(a, GetUserFolder() + "mods/multiplayer" + a.substr(a.find_last_of('/')),
		...
		if(!Terminate){
			...
			fs::copy_file(a,GetUserFolder() + "mods/multiplayer" + FName, fs::copy_options::overwrite_existing);
		}
		WaitForConfirm();
	}
	KillSocket(DSock);
